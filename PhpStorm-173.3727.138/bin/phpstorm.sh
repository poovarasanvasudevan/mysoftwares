...
if [ -z "$JDK" -a -s "$HOME/.PhpStorm2017.3/config/phpstorm.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.PhpStorm2017.3/config/phpstorm.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.PhpStorm2017.3/config/phpstorm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PhpStorm2017.3/config/phpstorm$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$AGENT_PATH" ]; then
    AGENT="-agentpath:$AGENT_PATH=disablealloc,delay=10000,probe_disable=*,sessionname=PhpStorm2017.3"
  fi
fi
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=PhpStorm2017.3 \
